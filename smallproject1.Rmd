...
From the density plot generated above, it is apparent that afternoon CH4 concentration during spring season seems to present the highest peak of concentration from the range of 1900 ppb to 1970 ppb (relatively mild stage from the given scale). The fall and summer seasons show relatively the same density plot, meaning that the average measured atmosphere methane concetrations in both seasons are about the same. Lastly, in winter season, the concentration is generally weaker throughout the season compared to others; however, the high concentration which ranges above 1950 ppb is more easily detectable.

```{r boxplot, fig.align="center"}
ggplot(season.noon, aes(x = Season, y = Obs)) + geom_boxplot(aes(fill = Season))
```

From the boxplots created above, it seems like fall and summer show similar widths of methane concentration level (ppb). As shown in density plot above, winter boxplot clearly demonstrates the higher ppb range. Plus, we are able to see all four seasons certainly include outliers, which definitely affects our qq-plots and residual plots as well. When it comes to normality, all the seasons but winter show a much less-skewness (as the medians are centered).

### ii)

```{r time-series, fig.align="center"}
ggplot(CH4.ave, aes(Date, CH4.ave[2])) + geom_point() + geom_line() + geom_smooth(span = 0.4) + 
...
ggplot(CH4.ave.lm.df, aes(Date.n, .resid)) + geom_point() + geom_smooth() +
...
From the time series plot for daily CH4 observation at COP above, daily CH4 describes highest pattern during winter (December, January and February) than other three seasons. During summer season (June, July and August), daily CH4 shows lowest pattern. From the residual plot of the loess trend fit, the oscillations are contained in the residuals since they are not fitted by the trend curve.

The trend component was used to describe deeply to interpret time series data. Interpreting the oscillations plot, it is hard to observe any strong pattern or obvious oscillations because there is only one oscillation in whole graph which make hard to figure out the cycle. So, we can conclude that there is not quite strong trend components beside the time series plot. The residuals also show random pattern.  
...
Basically, both qqplots clearly do not seem to have be normal distibution of residuals especially at the first tail. However, as shown above, this is due to the great amount of outliers in both dataset. Thus, it is actually normal to have non-normality in both cases, meaning that the weird looking plots are actually okay as they are expected.
